...
from BaseClasses.simulation_base import ObservationThresholdContinuousSimulation, OptimalPolicySimulation
...
def create_simulation(sim_type, battery_capacity, threshold, wind_threshold, horizon, data_path):
    ...
    x = np.linspace(np.pi, np.pi * 10, horizon)
    ...
    if sim_type == "threshold":
        sim = ObservationThresholdContinuousSimulation(
            mdp=mdp,
            horizon=horizon,
...
def main(config_file):
    ...
    run_manager.run_simulations(simulations, use_multiprocessing=False)
