...
def plot_first_year_weather_data(file1, file2):
    ...
    # Wind Speed Plot (both files)
    axes[0, 0].plot(data1_first_year.index, data1_first_year['wind_speed_10m'], label=f'Synthetic Data ({first_year_data1})', color='b')
    axes[0, 0].set_title('Wind Speed (Synthetic Data)')
    axes[0, 0].set_xlabel('Time')
    axes[0, 0].set_ylabel('Wind Speed (m/s)')
    axes[0, 0].legend()

    axes[0, 1].plot(data2_first_year.index, data2_first_year['wind_speed_10m'], label=f'Recorded Data ({first_year_data2})', color='r')
    ...
    # Wind Direction Plot (both files)
    axes[1, 0].plot(data1_first_year.index, data1_first_year['wind_direction_10m'], label=f'Synthetic Data ({first_year_data1})', color='b')
    axes[1, 0].set_title('Wind Direction (Synthetic Data)')
    axes[1, 0].set_xlabel('Time')
    axes[1, 0].set_ylabel('Wind Direction (°)')
    axes[1, 0].legend()

    axes[1, 1].plot(data2_first_year.index, data2_first_year['wind_direction_10m'], label=f'Recorded Data ({first_year_data2})', color='r')
    ...
    # Shortwave Radiation Plot (both files)
    axes[2, 0].plot(data1_first_year.index, data1_first_year['shortwave_radiation'], label=f'Synthetic Data ({first_year_data1})', color='b')
    axes[2, 0].set_title('Shortwave Radiation (Synthetic Data)')
    axes[2, 0].set_xlabel('Time')
    axes[2, 0].set_ylabel('Shortwave Radiation (W/m²)')
    axes[2, 0].legend()

    axes[2, 1].plot(data2_first_year.index, data2_first_year['shortwave_radiation'], label=f'Recorded Data ({first_year_data2})', color='r')
...
if __name__ == "__main__":
    ...
    plot_first_year_weather_data(r"Data\SYNTHETIC_DATA\data_30min_0.pkl", r"Data\EXPECTED_DATA\data_expected_30min.pkl")